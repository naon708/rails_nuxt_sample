module JwtAuthenticator
  ...
  def jwt_authenticate
    ...
    # render json: { error: "認証情報が不足しています" } if request.headers['Authorization'].blank?

    # トークンをヘッダーから取得する
    # headers['Authorization'] = "Bearer XXXXX..."
    encoded_token = request.headers['Authorization'].split('Bearer ').last
    # トークンを復号化する(トークンが復号できない場合、有効期限切れの場合はここで例外が発生する)
  ...
  # 暗号化処理
  def encode(user_id)
    expires_in = 2.weeks.from_now.to_i # 再ログインを必要とするまでの期間を１ヶ月とした場合
